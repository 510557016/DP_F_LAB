...
def model_analysis(text,num):
	...
	if(text=="LIFE"):
		type_selection = random.randint(1,100) % 5
		if(type_selection == 0):
			type_prediction = 'POLITICS'
		if(type_selection == 1):
			type_prediction = 'BUSINESS'
		if(type_selection == 2):
			type_prediction = 'STYLE & BEAUTY'
		if(type_selection == 3):
			type_prediction = 'WELLNESS'
		if(type_selection == 4):
			type_prediction = 'HEALTHY LIVING'
		if(type_selection == 5):
			type_prediction = 'HOME & LIVING'		
	elif(text=="FIFA2022"):
		type_selection = random.randint(1,100) % 4
		if(type_selection == 0):
			type_prediction = 'TRAVEL'
		if(type_selection == 1):
			type_prediction = 'ENTERTAINMENT'
		if(type_selection == 2):
			type_prediction = 'SPORTS'
		if(type_selection == 3):
			type_prediction = 'STYLE & BEAUTY'	 
	else:
		type_selection = random.randint(1,100) % 6
		if(type_selection == 0):
			type_prediction = 'A'
		if(type_selection == 1):
			type_prediction = 'B'
...
for num in range(sum_of_news):
	...
	#preds = hdd_pipe.predict(result[num]['desc'])
	#print(preds) 
	#模擬分析
